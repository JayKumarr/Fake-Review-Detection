 ...
 * @author mf15-10
 */
public class ReviewPrepareForLogisticRegressionBigram {
    public static void generateUnigrams(File inputFolder, File outputFolder, boolean printColumns, boolean printOriginal, boolean printFakeColumn) throws IOException {
        ...
        PrintStream outWords = new PrintStream(new FileOutputStream(new File(outputFolder,"bigram_words.csv")));
        PrintStream out = new PrintStream(new FileOutputStream(new File(outputFolder,"bigram.csv")));
...
//      and word whose index is 359 in hashtable have occured four times in ReviewPOJO 5 
    ...
    public static int addInMatrix(List<ReviewPOJO> reviewsList,int[][] matrix_fake_review, int cols, Hashtable<String, Integer> table){
        ...
        for(ReviewPOJO reviewPJO : reviewsList){
            ...
            List<String> lemmetizedList = reviewPJO.getLemmetized();
            for(int i=0; i< lemmetizedList.size()-1; i++){
                String word = lemmetizedList.get(i);
                String word2 = lemmetizedList.get(i+1);
                Integer dimension =  table.get(word+" "+word2);
    ...
    public static void printMatrixInFile(int[][] matrix_review, PrintStream out, String is_fake, int cols, boolean printFakeColumn){
    ...
    private static void addingWordsIntoList(ArrayList<File> fileList, List<ReviewPOJO> reviewsList, Set<String> words, StandfordLemmatizer lemmatizer) throws IOException {
        for(File f : fileList){
            ...
            for(int i=0; i< lemmatize.size()-1; i++){
                String word = lemmatize.get(i);
                String word2 = lemmatize.get(i+1);
